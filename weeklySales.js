exports.weeklySales = function(folderName) {
  ...
  inputSales2 = inputSales1.replace("Day,Date,stock item,No sold,Sales Price\n", "");
  inputSales3 = inputSales1.replace("Day,Date,stock item,No sold,Sales Price\n", "");
  inputSales4 = inputSales1.replace("Day,Date,stock item,No sold,Sales Price\n", ""); // removes the headings
  var interimArray1 = inputSales1.split('\n');
  var interimArray2 = inputSales1.split('\n');
  var interimArray3 = inputSales1.split('\n');
  var interimArray4 = inputSales1.split('\n'); //splits each itemsale by new line
  ...
  for (i = 1; i < salesList2.length; i++) { //maps stock item and amount sold.
    ...
    if (i % 7 === 0) {
      ...
      for (j = 0; j < 7; j++) {
        sold2 += salesList2[i + j][1]; //gets the No sold
      }
  ...
  for (i = 1; i < salesList4.length; i++) { //maps stock item and amount sold.
    ...
    if (i % 7 === 0) {
      ...
      for (j = 0; j < 7; j++) {
        sold4 += salesList4[i + j][1]; //gets the No sold
      }
  ...
  var weeklySales = [weeklySales1,weeklySales2,weeklySales3,weeklySales4];
